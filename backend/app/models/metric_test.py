...
async def register_test(test, db):
    ...
    try: 
        ...
        if not testInfo['info']['x-tests_metric']:
            return JSONResponse(status_code=500, content={'error': 'Missing the field info.x-tests_metric in the YAML'})
        if not testInfo['info']['x-applies_to_principle']:
            return JSONResponse(status_code=500, content={'error': 'Missing the field info.x-applies_to_principle in the YAML'})
