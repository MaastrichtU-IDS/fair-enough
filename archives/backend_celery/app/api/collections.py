...
@router.get(
    "/collections/{id}", response_description="Get a single collection", response_model=CollectionModel
)
...
@router.put("/collections/{id}", response_description="Update a collection", response_model=CollectionModel)
...
@router.post("/metrics/collections", response_description="Add a new collection", 
    response_model=CollectionModel, tags=['FAIR Metrics'])
async def create_collection(
        collection: CreateCollectionModel = Body(...),
        current_user: models.User = Depends(login.get_current_user)):
    collection = jsonable_encoder(collection)
    ...
    collec_obj = {
        ...
        'author': current_user['id'],
        # 'created': str(datetime.datetime.now().strftime("%Y-%m-%d@%H:%M:%S")),
        '@id': f'{settings.BASE_URI}/metrics/collection/{collection["_id"]}',
        '@context': settings.CONTEXT
    }
    # if 'homepage' in collection.keys() and collection['homepage']:
    #     collec_obj['homepage'] = collection['homepage']
